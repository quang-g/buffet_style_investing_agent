...
OUT_DIR = Path(os.getenv("CHUNK_OUT_DIR", str(DATA_DIR / "chunks_llm_gpt" / "localpos_v7_2_langextract")))
...
def _coerce_int(x: Any) -> Optional[int]:
    """Best-effort convert JSON numbers that may arrive as strings/floats into ints."""
    if isinstance(x, int):
        return x
...
def _build_ws_normalized_with_map(s: str) -> Tuple[str, List[int]]:
    """
    Collapse all whitespace runs to a single space, returning:
      - normalized string
      - map from each normalized char index -> raw char index
    """
    out_chars: List[str] = []
    idx_map: List[int] = []
    i = 0
    ...
    while i < n:
        ...
        if ch.isspace():
            ...
            while j < n and s[j].isspace():
                j += 1
            out_chars.append(" ")
            idx_map.append(i)  # map the normalized space to the first raw ws char
            i = j
        else:
            out_chars.append(ch)
            idx_map.append(i)
...
def _find_substring_offsets(window_text: str, chunk_text: str) -> Optional[Tuple[int, int]]:
    """
    Find chunk_text within window_text.
    Strategy:
      1) exact search
      2) whitespace-normalized exact search (maps back to raw)
      3) fuzzy longest-match fallback (requires large overlap)
    Returns (start, end) offsets in *window_text* (end exclusive), or None.
    """
    if not chunk_text:
        return None

    # 1) exact
    pos = window_text.find(chunk_text)
    if pos != -1:
        return (pos, pos + len(chunk_text))

    # 2) whitespace-normalized exact (stable, cheap)
    norm_win, win_map = _build_ws_normalized_with_map(window_text)
    norm_chunk, _ = _build_ws_normalized_with_map(chunk_text)
    pos = norm_win.find(norm_chunk)
    if pos != -1 and pos + len(norm_chunk) - 1 < len(win_map):
        start_raw = win_map[pos]
        end_raw = win_map[pos + len(norm_chunk) - 1] + 1
        return (start_raw, end_raw)

    # 3) fuzzy longest-match (as last resort)
    sm = difflib.SequenceMatcher(None, window_text, chunk_text)
    match = sm.find_longest_match(0, len(window_text), 0, len(chunk_text))
    # require substantial overlap to avoid random matches
    min_ok = max(40, int(0.6 * len(chunk_text)))
    if match.size >= min_ok:
...
- IMPORTANT: end_char is EXCLUSIVE (Python slicing).
- Include start_anchor and end_anchor (short snippets) for repair.
- DO NOT return chunk_text. Return only anchors + offsets + metadata.
...
def _map_window_offsets_to_global(
    ...
    *,
    ...
    for ch in window_chunks:
        # Accept a few possible key names from the LLM
        s_raw = ch.get("start_char", ch.get("start"))
        e_raw = ch.get("end_char", ch.get("end"))
        s = _coerce_int(s_raw)
        e = _coerce_int(e_raw)

        # If the LLM didn't return offsets, ground using anchors inside this window.
        if s is None or e is None:
            sa = (ch.get("start_anchor") or "").strip()
            ea = (ch.get("end_anchor") or "").strip()
            grounded = None
            if sa and ea:
                grounded = _repair_offsets_by_raw_anchors(window_text, sa, ea)
                if grounded is None:
                    grounded = _repair_offsets_by_normalized_anchors(window_text, sa, ea)
            if grounded is not None:
                s, e = grounded
                ch["offset_source"] = "grounded_from_anchors_in_window"
            else:
                ch["offset_source"] = "missing"
