...
- DO NOT include position_in_letter/position_in_section/total_chunks_in_section/offset_confidence.
        ...
        try:
            ...
            if not salvaged:
                raise RuntimeError(
                    ...
                    f"See raw response: {OUT_DIR / f'{year}_raw_llm_response.json'}"
                )
            ...
            print(f"[WARN] LLM call failed for {year} (attempt {attempt}/{retry.max_attempts}): {exc}")
            print(f"[INFO] Retrying in {sleep_s:.1f}s...")
            time.sleep(sleep_s)

    raise RuntimeError(f"LLM call failed for {year} after {retry.max_attempts} attempts: {last_exc}")
