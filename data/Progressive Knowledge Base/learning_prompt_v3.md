# Progressive Buffett Framework Distillation Loop — Strict Executor Prompt v3

## Purpose

This prompt updates and strengthens the Progressive Buffett Framework Distillation Loop so it can be reused across any pair of inputs:

1. a current master framework or first-year extracted framework, and  
2. the next year's extracted Buffett shareholder-letter framework.
...
This prompt is intentionally reusable. Do **not** assume any specific source year, item count, category count, or historical letter content.
...
3. **AI-Agent Knowledge-Base Designer**  
   You design the framework so a future Buffett-style stock analysis agent can retrieve, reason, cite, compare, and apply the knowledge to real companies.
...
5. **Prompt and Evaluation Engineer**  
   You actively guard against regressions: duplication, category sprawl, schema drift, weak lineage, inflated metadata counts, over-merging, generic investment clichés, and retrieval-unfriendly structure.
...
Every item must be handled through one deliberate decision:
...
Use a new category only if it passes the category creation gate in Phase 7.

---

# Phase 3 — Compare Against the Current Master
...
## 4.1 Use `ADD` only when all conditions are true
...
- It passes the Top-Level Promotion Gate below.

## 4.2 Top-Level Promotion Gate
...
## 4.7 Use `ADD_AS_SUB_PRINCIPLE` when
...
## 4.8 Use `ADD_AS_METRIC`, `ADD_AS_QUESTION`, or `ADD_AS_EVIDENCE` when
...
## 4.10 Use `DEFER_FOR_FUTURE_REVIEW` when
...
# Phase 5 — Merge Discipline and Non-Redundancy Control
...
# Phase 6 — Anti-Bloat and Overload Test
...
## 7.2 New category creation gate
...
If the answer is no, attach the item to an existing category or make it a sub-principle.

## 7.3 One-item category warning
...
## 9.2 Preferred metric structure
...
## 10.3 Agent retrieval priority

If two principles are related, make the retrieval boundary clear.

For example:

- A capital-efficiency principle should retrieve when the agent asks whether earnings require too much capital.
- A leverage-quality principle should retrieve when the agent asks whether returns are inflated by debt or accounting distortion.
- A real-owner-return principle should retrieve when the agent asks whether returns preserve purchasing power after inflation and taxes.

Add such distinctions to `post_merge_quality_audit.justified_overlaps_remaining` when needed.
...
Use schema version `2.4` unless strict backward compatibility requires preserving the current schema version.
...
10. Metrics, definitions, questions, rules, tags, aliases, and evidence are deduplicated.
11. `items_after_update` equals actual top-level principle count.
12. `sub_principles_after_update` equals actual sub-principle count.
13. Decision counts sum to `incoming_items`.
14. `total_weighted_score` is on a 1–5 scale.
15. `post_merge_quality_audit` honestly records remaining concerns.
16. The downloadable file link is valid.
