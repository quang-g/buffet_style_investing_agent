...
# Phase 13 — Dynamic Preservation and Regression Test

Before final output, run a two-layer regression test.

This test must protect both:

1. canonical Buffett concepts already known to be important, and
2. emergent concepts discovered from processed source years.

Do not treat the canonical concept list as complete.
It is a starting seed, not a closed taxonomy.

---

## 13.1 Canonical Buffett Concept Regression

Check the following seed concepts:
...
- Do not force concepts not yet supported by processed years.
- If a concept has not appeared yet, mark NOT_YET_SUPPORTED_BY_SOURCE_YEARS.
- If an old supported concept disappears, mark FAIL and fix before finalizing unless it was intentionally consolidated with clear lineage.

---

## 13.2 Emergent Concept Preservation Test

After processing the incoming year, identify whether the new source introduces any concept that is:

- Buffett-faithful,
- reusable across companies or years,
- analytically important for stock analysis,
- not already covered by the canonical regression list,
- not merely an example, metric, transaction, or historical detail.

For each such concept, decide one of:

- ADD_TO_EMERGENT_REGRESSION_TRACKER
- MAP_TO_EXISTING_CANONICAL_CONCEPT
- MAP_TO_EXISTING_EMERGENT_CONCEPT
- DEFER_AS_CANDIDATE_CONCEPT
- REJECT_AS_NON_REUSABLE

An emergent concept should be added to the tracker only if it passes at least one of these tests:

1. It appears in multiple source years.
2. It materially refines a major Buffett idea.
3. It introduces a distinct analytical question useful for real company analysis.
4. Losing it would reduce the framework’s ability to analyze companies.
5. It is likely to recur in future Buffett letters or Berkshire decisions.

---

## 13.3 Dynamic Regression Registry

Maintain a field called `dynamic_regression_registry`.

The registry should contain both canonical and emergent concepts.
...
```json
{
  "concept_id": "",
  "concept_name": "",
  "concept_type": "CANONICAL|EMERGENT",
  "first_seen_year": 0,
  "source_item_ids": [],
  "mapped_principle_ids": [],
  "mapped_sub_principle_ids": [],
